    ...
    def parse(self, response):
        ...
        for product in products:
            ...
            yield {
                ...
                'old_price_reais': prices[0] if len(prices) > 0 else None,  #necessário ter o len pra erntender se tem tamanho de lista
                ...
                #nas listas selecionamos especificos dentro da lista oq é oq
        ...
        if self.page_count < self.max_pages:
            next_page = response.css('li.andes-pagination__button.andes-pagination__button--next a::attr(href)').get()  #em vez de pegar queremos oa tibuto ou seja a url
            if next_page:
                self.page_count += 1
                yield scrapy.Request(url=next_page, callback=self.parse) #quero que ele rode a função novamente com a proxima página que voce está pegando na pagina atual
